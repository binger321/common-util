...
/**
 * 现在遇到的问题是，从 feign.hystrix.enabled=true, 使用降级服务，
 * 服务间token转发的时候丢失，原因是 Hystrix 不转发；所以，需要补发，该类的逻辑就是取转发相关遗漏的 Credentials；
 * 在 OAuth 认证中是关键；
 * <p/>
 *
 */
...
public class HystrixCredentialsContext {
            ...
            public void apply(RequestTemplate requestTemplate) {
                ...
                if( auth != null ){
                    ...
                    if(auth.getDetails() instanceof String){
                        requestTemplate.header("Authorization", "bearer " + auth.getDetails() );
                    }else if(auth.getDetails() instanceof OAuth2AuthenticationDetails){
                    ...
                    }else if(auth instanceof MyScheduleAuthentication){
                        requestTemplate.header("Authorization", "bearer " +  (String)((MyScheduleAuthentication)auth).getDetails() );
                    }
                }else{
    ...
    public void initializeHystrixRequestContext(MyScheduleAuthentication auth){
        if(auth!=null) {
